...
pub fn render(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let (indicator_color, label) = match &app.summary {
        Some(s) => (app.theme.indicator_color(s.status.indicator), s.status.description.clone()),
    ...
    let title_line = Line::from(vec![
        ...
        Span::styled("· terminal status for claude.com", Style::default().fg(app.theme.muted())),
    ]);
    let status_line = Line::from(vec![
        Span::styled("● ", Style::default().fg(indicator_color).add_modifier(Modifier::BOLD)),
        Span::styled(label, Style::default().fg(app.theme.text()).add_modifier(Modifier::BOLD)),
    ]);
    frame.render_widget(
...
fn right_status(app: &App) -> Vec<Line<'static>> {
    let theme_line = Line::from(vec![
        ...
        Span::styled(app.theme.name.clone(), Style::default().fg(app.theme.accent())),
        Span::raw("  "),
        Span::styled("? help", Style::default().fg(app.theme.muted())),
    ]);
    let updated = match app.last_loaded_at {
        Some(t) => format!("↻ {}", relative(t)),
        None => "↻ never".to_string(),
    };
    let updated_line = Line::from(vec![
        Span::styled(updated, Style::default().fg(app.theme.dim())),
    ]);
